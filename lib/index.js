...
class Sentinet extends EventEmitter {
  ...
  start() {
    if (this.proc) return;
    // Build args so script is the first argument after the interpreter
    const args = [this.script];
    if (this.iface) args.push('--iface', this.iface);
    // Use -u for unbuffered output so we can stream stdout/stderr
    const spawnArgs = ['-u', ...args];
    this.proc = spawn(this.pythonPath, spawnArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
